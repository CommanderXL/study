...
但是在模板编译后，统一将`dom`节点(不管是built in的节点还是自定义component节点)上的属性转化为`attrs`对象(见代码片段111)，在创建`VNode`过程中调用了`extractPropsFromVNodeData`这个方法完成从`attrs`对象上获取到这个`component`所需要的`props`属性，获取完成后还会将`attrs`对象上对应的`key`值删除。因此这个key值对应的是要传入子`component`的数据，而非原生`dom`属性，最终由`VNode`生成真实`dom`的时候是不需要这些自定义数据的，因此需要删除。
