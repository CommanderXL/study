...
@vue/cli-service 将基于 webpack 的本地开发构建配置收敛至内部来实现，当你没有特殊的开发构建需求的时候，内部配置可以开箱即用，不用开发者去关心一些细节。当然在实际团队开发当中，内部配置肯定是无法满足的，得益于 @vue-cli@3.0 的插件构建设计，开发者不需要将内部的配置进行 eject，而是直接使用 @vue/cli-service 暴露出来的 api 去完成对于本地开发构建的改造和更新。
...
同时在插件系统内部也对核心功能进行了插件化的拆解，方便整个插件系统的拓展。
