...
> 由于代码召回的内容涉及到过多的技术细节，没法在文章里面呈现的面面俱到；
...
* **核心差异一：索引粒度是“函数体源码”还是“结构化 symbol 摘要”。**
  * FTS Index：
...
结合我们自己的业务实践来看，开源代码 RAG 产品直接落到业务前端仓库时，还会遇到一个更现实的问题：它们对业务框架的索引和 CodeGraph 支持并不充分。当前不管是 MindWiki、GitNexus，还是 CodeGraph 这类产品，核心能力大多围绕通用语言的 symbol、file path、函数调用关系展开，对 `mpx` / `vue` 这类 SFC 结构、模板到脚本的引用关系、组件注册以及 options api 的代码组织形式都没有支持。

基于这个问题，我们前段时间设计了一套面向前端代码场景的语义 CodeGraph，并在此基础上深度魔改了 MindWiki / GitNexus / CodeGraph 这3个产品的代码：

* **索引构建层面**：补齐 `mpx` / `vue` / `react` 的框架语义解析，支持页面、组件、模板、脚本、路由、事件等信息的索引构建；
* **召回策略层面**：尽量复用各产品本身已有的 FTS/BM25、Vector、hybrid search、rerank 等召回策略，避免重新设计整套 RAG pipeline；
* **图引擎层面**：将我们设计的语义 CodeGraph 作为各产品底层的图能力，让 MindWiki / GitNexus / CodeGraph 在做图扩展时，不再只沿普通代码调用关系扩展，而是可以沿业务框架语义关系扩展。

从 single Q&A benchmark 的结果来看，分别魔改上述3个产品代码后，CodeGraph 的召回效果相较于 AI-native 的 `grep` / `glob` 更优：


## 3. RAG 与业务语义

第二章简答聊了下 RAG 工具，主要解决的是“如何更快、更完整地从代码索引中召回相关上下文”。但它们能发挥多大作用，也比较依赖用户 query 和代码索引之间是否存在稳定的匹配关系。这里把 query 分成两类：**工程语义 query** 和 **业务语义 query**。

### 工程语义 query：RAG 擅长的场景

工程语义 query 指的是问题偏向明确的符号 symbol、工程术语，它们语义聚焦，和代码命名高度重合。例如：
...
业务语义 query 偏向业务场景和流程，某个语义词背后往往是多个页面、组件、函数和状态流转的集合，query 里面可能没有明确的代码锚点。例如：
...
* **内容是定位索引，不是代码细节。** 页面文档只记录入口组件、关键接口、Store、组件关系等可帮助定位的信息；可以直接从源码读到的实现细节不固化到文档里。知识库负责告诉 LLM “应该去哪里读代码”。
...
* 业务语义 query 可通过业务知识库定位代码锚点；
* 工程语义 query 通过调用 RAG 工具精确召回；
