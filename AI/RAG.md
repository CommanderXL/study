...
两者结合后，LLM 可以根据 query 类型选择合适的第一跳召回方式：
...
* 我们通过测试发现在 long-running task 当中，LLM 往往更倾向使用 bash shell 取代 grep/glob 的调用，实际能被 hook 的次数并不多（例如上面通过 cspec 执行的长任务，0次 grep/glob 调用）；
