...
这段注释虽然存在，但没有写出“防倒挂”这个业务词。bigram 只能从注释里抽到 `遍历 / 列表 / 最大值`，无法知道它和 `anti_price_reverse` 的业务关系。更糟的是，如果类似技术实现注释大量出现，系统可能会把“最大值”“列表”这类无关词也纳入候选映射，最终让召回结果变得更噪。

**局限二：很多业务问题不是一个函数能表达的。** bigram 最擅长建立的是下面这种映射：

```text
防倒挂 -> checkAntiPriceReverse
```

也就是一个业务词，最后落到一个比较明确的函数或类。但很多真实 query 不是在问“某个业务词对应哪个函数”，而是在问“一整条业务流程是怎么串起来的”。例如：
