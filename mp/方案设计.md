...
`element.mpx` 不同于一个普通的 `x.mpx` 文件的处理流程，`element.mpx` 最终输出的内容都是在编译环节收集起来并注入进去的，是纯静态的内容，所以不需要正常的处理流程，只需要 selector -> 对 `template/json/wxss` 注入编译环节收集的内容 -> importModule 输出内容。
