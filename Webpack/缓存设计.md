...
例如官方提供的 `cache-loader`，。。。，但是 `cache-loader` 也仅仅能覆盖到经由 `loader` 处理后的文件内容，缓存内容的范围比较受限。另外就是 `cache-loader` 是通过对比文件 metadata 的 timestamps，这种缓存失效策略不是非常的安全，具体见 case。
